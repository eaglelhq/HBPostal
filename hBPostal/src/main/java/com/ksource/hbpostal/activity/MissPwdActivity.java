...
public class MissPwdActivity extends BaseActivity {
	...
	private void checkAuthCode() {
		...
		paramValues.put("phone", et_phone_number.getText().toString()
				.trim());
		...
		StringCallback callback = new StringCallback() {
			...
			public void onResponse(String arg0, int arg1) {
				...
				if (pwdResult == null) {
					ToastUtil.showTextToast(context, "检查验证码失败！");
					return;
				}
		...
		DataUtil.doPostAESData(mLoadDialog,context,ConstantValues.CHECK_CODE_URL, paramValues, callback);
	}
...
//		et_phone_number.setText(PhoneUtils.getPhoneNumber(context));
		...
		if (type == 1){
			...
			codeUrl = ConstantValues.GET_RESET_URL;
		}else{
			tv_title.setText("重置支付密码");
			codeUrl = ConstantValues.GET_RESET_PWD_CODE;
		}
	}
	...
	private void getYZM() {
		...
		DataUtil.doPostAESData(mLoadDialog,context,codeUrl, paramValues, callback);
	}
