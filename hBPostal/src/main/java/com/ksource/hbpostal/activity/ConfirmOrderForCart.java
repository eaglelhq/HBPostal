...
//		tv_jifen = (TextView) findViewById(R.id.tv_jifen);
	...
	public void initData() {
		...
		datas = (List<GoodsBean>) bundle.getSerializable("orderList");
		for (int i = 0; i < datas.size(); i++) {
			money = NumberUtil.add(money, NumberUtil.mul(
					ConvertUtil.obj2Double(datas.get(i).PRICE),
					ConvertUtil.obj2Double(datas.get(i).NUMBER)));
			jifen = NumberUtil.add(jifen, NumberUtil.mul(
					ConvertUtil.obj2Double(datas.get(i).INTEGRAL),
					ConvertUtil.obj2Double(datas.get(i).NUMBER)));
			number += datas.get(i).NUMBER;
		}
		getDefaultAddr();
		//默认配送方式
	...
	private void setCount() {
		...
		tv_postal_money.setText(NumberUtil.toDecimal2(sendPrice) + "元");
//		tv_money.setText("￥" + NumberUtil.toDecimal2(money));
//		tv_jifen.setText("" + jifen);
		if (money == 0) {
			tv_money.setText(ConvertUtil.obj2Int(jifen) + "积分");
		} else if (jifen == 0) {
			tv_money.setText("￥" + NumberUtil.toDecimal2(money));
		} else {
			tv_money.setText("￥" + NumberUtil.toDecimal2(money) + "+" + ConvertUtil.obj2Int(jifen) + "积分");
		}
	}
	...
	private void getText() {
		...
		for (GoodsBean bean : datas) {
			goodIds += bean.GOODS_ID + ",";
			buyNums += bean.NUMBER + ",";
		}
		goodIds.substring(0, goodIds.length() - 1);
		buyNums.substring(0, buyNums.length() - 1);
	...
	private void submitOrder() {
		...
		StringCallback callback = new StringCallback() {
			...
			public void onResponse(String arg0, int arg1) {
				...
				try {
					subResult = gson.fromJson(arg0, SubOrderResultBean.class);
				} catch (Exception e) {
					e.printStackTrace();
				}
				if (subResult == null) {
				...
				if (subResult.success) {
					...
					String goodsName = "";
					for (GoodsBean goodsBean : datas) {
						goodsName += goodsBean.NAME
								+ ",";
					}
	...
	private void getDefaultAddr() {
		...
		StringCallback callback = new StringCallback() {
			...
			public void onResponse(String arg0, int arg1) {
				...
				try {
					Gson gson = new Gson();
					addrResult = gson.fromJson(arg0, DefAddrResaultBean.class);
				} catch (Exception e) {
					e.printStackTrace();
				}
				if (addrResult == null) {
				...
				if (addrResult.success) {
					...
					if (!TextUtils.isEmpty(addrResult.address.PROVINCE_NAME)
							&& !TextUtils.isEmpty(addrResult.address.ADDRESS)) {
					...
					if (!TextUtils.isEmpty(addrResult.address.PROVINCE_ID)) {
		...
		DataUtil.doPostAESData(mLoadDialog,context,ConstantValues.GET_DEFAULT_ADDR, params,
				callback);
	...
	private void getPostMoney() {
		...
		StringCallback callback = new StringCallback() {
			...
			public void onResponse(String arg0, int arg1) {
				...
				try {
					Gson gson = new Gson();
					postalResult = gson.fromJson(arg0, PostalResultBean.class);
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				if (postalResult.success) {
					...
					tv_money.setText("￥" + NumberUtil.toDecimal2(NumberUtil.add(money, sendPrice)));
				} else {
		...
		DataUtil.doPostAESData(mLoadDialog,context,ConstantValues.GET_POST_MONEY_URL, params,
				callback);
	}
