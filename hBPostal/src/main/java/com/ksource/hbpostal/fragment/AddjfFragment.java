...
import com.ksource.hbpostal.activity.JifenActivity;
...
public class AddjfFragment extends BaseFragment {
		...
		StringCallback callback = new StringCallback() {
			...
			public void onResponse(String arg0, int arg1) {
				...
				if (resultBean.success) {
					...
					rl_error.setVisibility(View.GONE);
					JifenActivity.tv_jifen
							.setText(resultBean.memberScore + "积分");
					if (isUpdate) {
						datas.clear();
					}
...
//			if (position > 0) {
//				lastBeginTime = TimeUtil
//						.formatTimeMon(datas.get(position - 1).BEGIN_TIME);
//			}
//			String beginTime = TimeUtil
//					.formatTimeMon(datas.get(position).BEGIN_TIME);
//			String endTime = TimeUtil
//					.formatTimeDayHZ(datas.get(position).OVER_TIME);
