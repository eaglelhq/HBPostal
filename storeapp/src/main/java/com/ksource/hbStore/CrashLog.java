...
public class CrashLog implements UncaughtExceptionHandler {
	...
	private static final String CRASH_LOG = Environment.getExternalStorageDirectory().getPath() + "/com.ksource.hbpostal/crashlog/";
	...
	private boolean handleException(Throwable ex) {
		...
		// 保存日志文件
		if (IsWriteToFile) {
			String crashInfo = saveCrashInfo2File(ex);
			//TODO 把错误日志上传到服务器
		}
		return true;
	}
	...
	public static void catchException(Throwable ex) {
		...
		try {
			long timestamp = System.currentTimeMillis();
			String time = formatter.format(new Date());
			String fileName = "catchedException-" + time + "-" + timestamp + ".log";
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				String path = CRASH_LOG;
				File dir = new File(path);
				if (!dir.exists()) {
